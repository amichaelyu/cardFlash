...
// class _SettingsPageState extends State<_SettingsPage> {
  final Uri _developer = Uri.parse('https://github.com/amichaelyu');
  final Uri _privacyPolicy = Uri.parse('https://github.com/amichaelyu/cardFlash/blob/master/PRIVACY.md');
  final Uri _bugReport = Uri.parse('https://amichaelyu.github.io/cardFlashBug/');
  final Uri _featureRequest = Uri.parse('https://amichaelyu.github.io/cardFlashFeature/');
  // final Uri _betaTester = Uri.parse('https://amichaelyu.github.io/cardFlashBeta');
          ...
          if (snapshot.data != null) {
            return ListView(
                children: [
                  ...
                  Text("Settings", semanticsLabel: "Settings", textAlign: TextAlign.center, style: TextStyle(fontSize: MediaQuery.of(context).size.height * 0.024, fontWeight: FontWeight.bold)),
                  const Padding(
                  ...
                  Text("About", semanticsLabel: "About", textAlign: TextAlign.center, style: TextStyle(fontSize: MediaQuery.of(context).size.height * 0.024, fontWeight: FontWeight.bold)),
                  const Padding(
                    padding: EdgeInsets.only(top: 10),
                  ),
                  BetterCardSettings("Version: ${Constants.version}+${Constants.build}", null, null),
                  ...
                  Text("Legal", semanticsLabel: "Legal", textAlign: TextAlign.center, style: TextStyle(fontSize: MediaQuery.of(context).size.height * 0.024, fontWeight: FontWeight.bold)),
                  const Padding(
                    padding: EdgeInsets.only(top: 10),
                  ),
                  ...
                  Text("Reset", semanticsLabel: "Reset", textAlign: TextAlign.center, style: TextStyle(fontSize: MediaQuery.of(context).size.height * 0.024, fontWeight: FontWeight.bold)),
                  const Padding(
                    padding: EdgeInsets.only(top: 10),
                  ),
                        ...
                        builder: (BuildContext context) => AlertDialog(
                          ...
                          actions: <Widget>[
                            TextButton(
                              ...
                              child: const Text('Cancel', semanticsLabel: "Cancel", style: TextStyle(fontWeight: FontWeight.bold),),
                            ),
                            TextButton(
                              onPressed: () async {
                                final navigator = Navigator.of(context);
                                await (await SharedPreferences.getInstance()).setBool("rebirth", true);
                                await (await SharedPreferences.getInstance()).setBool("firstTime", false);
                                if (!mounted) return;
                  ...
                  BetterCardSettings("Reset App With Tutorial",
                          () {showDialog<String>(
                        context: context,
                        builder: (BuildContext context) => AlertDialog(
                          shape: const RoundedRectangleBorder(
                          ...
                          actions: <Widget>[
                            ...
                            TextButton(
                              onPressed: () async {
                                final navigator = Navigator.of(context);
                                await (await SharedPreferences.getInstance()).setBool("rebirth", true);
                                await (await SharedPreferences.getInstance()).setBool("firstTime", true);
                                if (!mounted) return;
                                Phoenix.rebirth(navigator.context);
                              },
                              child: const Text(
