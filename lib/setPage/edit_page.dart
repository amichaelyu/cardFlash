...
class _EditPageState extends State<EditPage> {
  ...
  Future<dynamic> _grabSomeData() async {
    ...
    _icon = IconData(snapshot[0]['iconCP'],
        ...
        fontPackage:
            snapshot[0]['iconFP'] == "" ? null : snapshot[0]['iconFP']);
  ...
  Widget build(BuildContext context) {
    return PopScope(
                ...
                Padding(
                    ...
                    child: ExpandTapWidget(
                      ...
                      onTap: () async {
                        ...
                        if (data.length - 1 == cardNum) {
                          for (int i = 0; i < cardNum; i++) {
                            if (!ignoreList.contains(i)) {
                              if (((terms?.object[i] != null) &&
                                      (terms?.object[i] != data[i + 1]['term'])) ||
                                  ((defs?.object[i] != null) &&
                                      (defs?.object[i] != data[i + 1]['def']))) {
                                termDefChanged = true;
                                break;
                              }
                            }
                        ...
                        if (termDefChanged ||
                            (data.length - 1 != cardNum) ||
                            ((title.text != "") && (title.text != data[0]['title'])) ||
                            ((desc.text != "") && (desc.text != data[0]['desc'])) ||
                            ((_icon?.codePoint != data[0]['iconCP']) &&
                                (_icon?.fontPackage != data[0]['iconFP']) &&
                                (_icon?.fontFamily != data[0]['iconFF']))) {
                          if (context.mounted) {
                            showDialog<String>(
                            context: context,
                            builder: (BuildContext context) => AlertDialog(
                ...
                child: CustomScrollView(slivers: [
                  SliverList(
                    delegate: SliverChildBuilderDelegate(
                      ...
                      (context, index) => [
                        BetterTextFormField("Enter a title", null, true,
                            "A title is required", null, null, title),
                        BetterTextFormField("Enter a description (Optional)",
                            null, false, null, null, null, desc),
                  ...
                  SliverReorderableList(
                    onReorder: (int oldIndex, int newIndex) {
                      ...
                      terms?.object[newIndex] =
                          tempTerms[oldIndex] ?? snapshot[oldIndex + 1]['term'];
                      defs?.object[newIndex] =
                          tempDefs[oldIndex] ?? snapshot[oldIndex + 1]['def'];
                      if (newIndex < oldIndex) {
                        for (int i = newIndex; i < oldIndex; i++) {
                          terms?.object[i + 1] =
                              tempTerms[i] ?? snapshot[i + 1]['term'];
                          defs?.object[i + 1] =
                              tempDefs[i] ?? snapshot[i + 1]['def'];
                        }
                      ...
                      for (int i = 0; i < terms?.object.length; i++) {
                        termCont[i].text =
                            terms?.object[i] ?? snapshot[i + 1]['term'];
                        defCont[i].text =
                            defs?.object[i] ?? snapshot[i + 1]['def'];
                      }
                    },
                  ...
                  for (int i = 0; i < cardNum; i++) {
                    if (!ignoreList.contains(i)) {
                      termsList.add(terms?.object[i] ??
                          (data.length - 1 > i ? data[i + 1]['term'] : ""));
                      defsList.add(defs?.object[i] ??
                          (data.length - 1 > i ? data[i + 1]['def'] : ""));
                    }
                  }
                  if (_formKey.currentState!.validate()) {
                    await LocalDatabase.updateSet(CardSet(data[0]['position'],
                        title.text, desc.text, _icon!, termsList, defsList));
        ...
        // onPopInvoked: (false) async => !Navigator.of(context).userGestureInProgress);
  }
}